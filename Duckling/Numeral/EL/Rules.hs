...
ruleNumeral = Rule
  ...
  where
    regexString = "(" ++ intercalate "|"
      [ "μηδ[εέ]ν|[εέ]ν[αοό]ς?|μ[ιί]ας?"                                -- [0..1]
      , "δ[υύ]ο|τρ(ία|εις)|τ[ςέ]σσερ(α|ις)|π[εέ]ντε"                       -- [2..5]
      , "[εέ]ξι|ε[πφ]τ[αά]|ο[κχ]τ[ωώ]|ενν(ιά|έα|εα|ια)|δ[εέ]κα|δεκαρι[αά]"                -- [6..10]
      , "[εέ]ν[τδ]εκα|δ[ωώ]δεκα|ντουζ[ιί]ν(α|ες)"                             -- [11..12]
      , "δεκα(τρ[ιί]α|τ[εέ]σσερα|π[εέ]ντε|[εέ]ξι|ε[πφ]τ[αά]|ο[χκ]τ[ωώ]|ενν([εέ]α|ι[αά]))"    -- [13..19]
      , "ε[ιί]κοσι|(τρι[αά]|σαρ[αά]|πεν[ηή]|εξ[ηή]|εβδομ[ηή]|ογδ[οό]|ενεν[ηή])ντα"           -- [2..9]0
      ] ++ ")"
...
ruleHundreds = Rule
  { name = "number (200..900)"
  , pattern =
      [ regex "(δι|τρι|τετρ|πεντ|εξ|ε(π|φ)τ|ο(χ|κ)τ|εννι)ακ[οό]σι(α|ες|οι)"
      ]
  , prod = \tokens -> case tokens of
      (Token RegexMatch (GroupMatch (match:_)):_) ->
...
rulePowersOfTen = Rule
  { name = "powers of tens"
  , pattern =
    [ regex "(χ[ιί]λι(α|οι|ες)|χιλι[αά]δες|εκατομμ[υύ]ρι(ο|α)|δις|δισεκατομμ[υύ]ριο)"
    ]
  , prod = \tokens -> case tokens of
